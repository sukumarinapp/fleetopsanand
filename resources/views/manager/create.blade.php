          ...
          <div class="col-sm-6">
			...
			<div class="card-body">
				...
				<div class="col-sm-9">
                  <select name="parent_id" id="parent_id" required="required" class="form-control select2" style="width: 100%;">
                    @foreach($managers as $manager)
                      <option data-settings="{{ $manager->BPA }},{{ $manager->BPB }},{{ $manager->BPC }},{{ $manager->BPD }},{{ $manager->BPE }},{{ $manager->BPF }},{{ $manager->BPG }},{{ $manager->BPH }},{{ $manager->BPI }},{{ $manager->BPJ }},{{ $manager->BPJ1 }},{{ $manager->BPJ2 }},{{ $manager->BPK }},{{ $manager->BPL }}" value="{{ $manager->id }}" >{{ $manager->UAN }} {{ $manager->name }} {{ $manager->UZS }}</option>
                    @endforeach
                  </select>
                </div>
                </div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-4">
						<input required="required" type="text" class="form-control number" name="UCN" id="UCN" maxlength="15" placeholder="Contact Number">
